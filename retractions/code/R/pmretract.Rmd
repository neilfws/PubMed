...
### 5.1 Setup
This code loads required libraries, searches PubMed for retracted articles, downloads the results as XML and returns a parsed XML document.
...
retracted <- fetchXML("\"Retracted Publication\"")
...
plotByYear <- function() {
  ...
  hc <- Highcharts$new()
  hc$title(text = "Retracted publications per 100 000 publications by year of publication")
  hc$series(data = as.numeric(sprintf("%.3f", (100000 / y$total) * y$retracted)), type = "column", events = list(click = "#! function() {window.open(this.options.url)} !#"))
  hc$xAxis(categories = y$year, labels = list(rotation = 90, formatter = "#! function() { return '<a href=\"http://www.pubmed.org/?term=%22Retracted Publication%22[PTYP] AND ' + escape(this.value) + '[DP]\" target=\"_blank\">' + this.value + '</a>'; } !#", useHTML = "true"), title = list(text = "year"))
