...
```{r get-data}
commons <- read_csv("../../data/commons.csv", col_names = FALSE)
...
```{r count-comments}
# all comments
commons.sum <- data.frame(`total records` = nrow(commons), 
                          `final dataset` = nrow(commons.real), 
                          moderated = nrow(commons[!is.na(commons$moderated), ]), 
                          `unique articles` = length(unique(commons.real$pmid)), 
                          `unique authors` = length(unique(commons.real$author.name)), 
                          stringsAsFactors = FALSE)
#ymax <- round(nrow(commons)+5, -1)
#commons.sum %>% 
#  gather(key, value) %>% 
#  ggplot(aes(reorder(key, value, max), value)) + geom_col(fill = "#336699", width = 0.4) + 
#    geom_text(aes(label = value), vjust = -0.2) + scale_y_continuous(breaks = seq(0, ymax, 1000), limits = c(0, ymax)) + 
#    labs(y = "total", x = "category", title = "Total PubMed Commons Comments By Sub-categories")
...
commons.real %>% 
  ...
  group_by(year) %>% 
  summarise(total = n()) %>% 
  ggplot(aes(year, total)) + geom_col(fill = "#336699", width = 0.4) + 
    labs(y = "comments", title = "Total PubMed Commons Comments By Year") + 
...
commons.real %>% 
  ...
  group_by(month, year) %>% 
  summarise(total = n()) %>% 
  ggplot(aes(month, total)) + geom_col(fill = "#336699") + 
    facet_grid(year ~ .) + scale_x_continuous(breaks = seq(1, 12, 1)) + 
    labs(y = "comments", title = "Total PubMed Commons Comments By Year & Month")
```
...
```{r comments-day-year}
commons.real %>% 
  ggplot(aes(yday(datetime))) + geom_bar(fill = "#336699") + facet_grid(year(datetime) ~ .) + 
    labs(x = "day", y = "comments", title = "PubMed Commons Comments By Day & Year")
```
...
commons.real %>% 
  mutate(date_new = as.Date(datetime)) %>%
  count(date_new) %>%
  arrange(desc(n)) %>%
  select(date = date_new, comments = n) %>%
  slice(1:5) %>%
  kable()
```
...
commons.real %>%
  ggplot(aes(pubyear)) + geom_bar(fill = "#336699") + 
    scale_x_continuous(breaks = seq(low, high, 10)) + 
    labs(x = "year of publication", y = "comments", title = "Total PubMed Commons Comments By Year Of Article Publication")
```
...
commons.real %>% 
  ggplot(aes(factor(year(datetime)), pubyear)) + geom_jitter(color = "#336699", alpha = 0.5) + 
    labs(x = "year comment posted", y = "year article published", title = "Distribution Of Publication Year For PubMed Articles With Comments") + 
    geom_violin(alpha = 0) + scale_y_continuous(breaks = seq(low, high, 10))
```

### Comments per article
```{r comments-article}
commons.pmid <- commons.real %>% 
  group_by(pmid) %>% summarise(count = n())
...
commons.pmid %>% 
  ggplot(aes(count)) + geom_density(color = "#336699", fill = "#336699") + 
    labs(x = "comments", title = "Distribution Of PubMed Commons Comments Per Article")
```

### Comments per author
```{r comments-author}
commons.auth <- commons.real %>% 
...
commons.auth %>% 
  ggplot(aes(count)) + geom_density(color = "#336699", fill = "#336699") + 
    labs(x = "comments", title = "Distribution Of PubMed Commons Comments Per Author")
```
...
```{r votes-density}
commons.real %>% 
  ggplot(aes(votes.down, votes.up)) + geom_bin2d(bins = 50) + 
...
commons %>%
  count(moderated) %>%
  filter(!is.na(moderated)) %>% 
  ggplot(aes(moderated, n)) + geom_col(fill = "#336699", width = 0.4) + 
  geom_text(aes(label = n), vjust = -0.2) + 
  labs(x = "removed by", y = "comments", title = "Moderation Of PubMed Commons Comments")
```
